...
module.exports.command = "follow <channels...>";
module.exports.describe = "Follow the target channels";
module.exports.builder = {
  channels: {
    coerce: arg => arg.map(a => a.toString().toLowerCase()),
  },
};
module.exports.handler = async function followHandler (argv) {
  const users = await argv.twitchApi.fetchUsers(argv.channels);

  for (const user of users) {
    await argv.twitchApi.follow(user._id);
  }

  argv.reply(`Followed ${argv.channels.join(", ")}`);
  logger.log("followed", argv.channels);
};
